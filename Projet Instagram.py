...
class InstagramDetectiveTool:
    ...
    def plot(self, month_counts):
        ...
        plt.title(f'Instagram Impact: {self.owner_name}')
...
if __name__ == "__main__":
    ...
    # -*- coding: utf-8 -*-
"""
Created on Wed Jan  7 12:49:05 2026
...
# Vérification rapide du volume de chaque base
print("\nTAILLE DES DIFFERENTS ENSEMBLES DE DONNEES :")
print("Likes de publications :", None if donnees_likes_publications is None else len(donnees_likes_publications))
print("Likes de stories :", None if donnees_likes_stories is None else len(donnees_likes_stories))
print("Likes de commentaires :", None if donnees_likes_commentaires is None else len(donnees_likes_commentaires))
print("Commentaires sous publications :", None if donnees_commentaires_publications is None else len(donnees_commentaires_publications))
print("Publications enregistrées :", None if donnees_publications_enregistrees is None else len(donnees_publications_enregistrees))
...
evolution_jour = df_likes_exp.groupby(df_likes_exp["date_action"].dt.date).size()

plt.figure(figsize=(10,5))
evolution_jour.plot(kind='line', marker='o')
plt.title("Évolution journalière des likes de publications")
plt.xlabel("Date")
...
plt.title("Top 10 comptes auxquels je like le plus")
plt.ylabel("Nombre de likes")
plt.show()
